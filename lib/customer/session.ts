...
export const CUSTOMER_SESSION_COOKIE = "neo4j_dashboard_customer_session";
...
export async function verifyCustomerSessionToken(
  ...
  const maybe = payload as Partial<CustomerSessionPayload>;
  if (maybe.sub !== "customer") return null;
  if (typeof maybe.exp !== "number") return null;
  if (typeof maybe.customerId !== "string" || !maybe.customerId) return null;
  if (typeof maybe.email !== "string" || !maybe.email) return null;
